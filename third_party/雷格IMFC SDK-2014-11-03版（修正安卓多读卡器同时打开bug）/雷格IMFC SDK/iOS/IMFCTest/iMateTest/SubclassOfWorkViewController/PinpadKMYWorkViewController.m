...
@interface PinpadKMYWorkViewController () <iMateAppFaceDelegate>

@property (strong, nonatomic) iMateAppFace *imateAppFace;

@end

@implementation PinpadKMYWorkViewController
...
- (void)viewDidLoad
{
    ...
    //设置Pinpad型号
    [_imateAppFace pinpadSetModel:PINPAD_MODEL_KMY];
    
    [self showLogMessage:@"凯明扬Pinpad测试项目："];
        ...
        if (nextStepType > PinPadRequestTypePowerOn) {
            switch (nextStepType) {
                ...
                case PinPadRequestTypeDownloadMasterKey:
                    ...
                    break;
                case PinPadRequestTypeDownloadWorkingKey:
                    [self.imateAppFace pinPadDownloadWorkingKey:YES masterIndex:1 workingIndex:1 workingKey:workingKey keyLength:16];
                    break;
                case PinPadRequestTypeInputPinBlock:
